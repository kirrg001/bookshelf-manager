 ...
 * @TODO:
 * - add belongsTo tests
 * - tidy up code, group into more functions
            ...
            .then((targets) => {
                ...
                return Promise.each(targets.models, (target) => {
                    if (!existingRelations.findWhere({id: target.id})) {
                        existingRelations.on('creating', (collection, data) => {
                            if (this.pluginOptions && this.pluginOptions.belongsToMany && this.pluginOptions.belongsToMany.beforeRelationCreation) {
                                return this.pluginOptions.belongsToMany.beforeRelationCreation(collection, data, options);
                            }
                        });
                    ...
                    if (this.pluginOptions && this.pluginOptions.belongsToMany && this.pluginOptions.belongsToMany.after) {
                        return this.pluginOptions.belongsToMany.after(existingRelations, targets, options);
                    }
                });
            })
            .then(() => {
    ...
    let Model = bookshelf.Model.extend({
        initialize: function initialize() {
            ...
            this.on('saved', (model, attributes, options) => {
                ...
                return bookshelf.manager.updateRelations(model, relations, options, opts.hooks)
                    .then(() => {
                        this.relationships.forEach((key) => {
                            if (this['relationships_' + key]) {
