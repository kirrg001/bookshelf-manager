            ...
            return Promise.each(targets.models, function (target) {
                if (!existing.findWhere({id: target.id})) {
                    existing.on('creating', function (collection, data) {
                        if (self.pluginOptions && self.pluginOptions.belongsToMany && self.pluginOptions.belongsToMany.beforeRelationCreation) {
                            return self.pluginOptions.belongsToMany.beforeRelationCreation(collection, data);
                        }
                    });
                ...
                if (self.pluginOptions && self.pluginOptions.belongsToMany && self.pluginOptions.belongsToMany.after) {
                    return self.pluginOptions.belongsToMany.after(existing, targets);
                }
            });
        }).then(function () {
            existing.off('creating');
            return model;
        });
});
...
Manager.prototype.updateRelations = function updateRelations(model, relations, options, pluginOptions) {
