...
class Manager {
    ...
    setHasOne(model, key, value, relation, options) {
        let Target = relation.relatedData.target;
        let existing = Target.forge(model.related(key).attributes);
        let target = Target.forge(value);
        let fk = relation.relatedData.foreignKey;
        let opts = _.cloneDeep(options);
        let isNew = existing.isNew();
    ...
    setCollection(existing, models, options) {
        ...
        return Promise.each(models, (properties) => {
            ...
            let model = exists ? exists : existing.model.forge(properties);
            let opts = _.cloneDeep(options);
